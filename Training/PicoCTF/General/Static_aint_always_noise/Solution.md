...
## Solution
From the description, we know that there is two file we need to download : a binary file and a BASH scrtip file.

After downloaded, we check the properties of the binary file, it is an executable file.
Let's try to run it,
```
chmod +x static
...
./static
```
Then, we got the output that telling us the flag is here somewhere :
```
Oh hai! Wait what? A flag? Yes, it's around here somewhere!

```

Next, let's try to run the bash script with the **static** file.
```
bash ltdis.sh static
```
We can get the output after running the bash script :
```
...
There are two txt files generated after it : static.ltdis.strings.txt and static.ltdis.x86_64.txt
...
The flag at the line 15 which is clearly show.
```
 1020 picoCTF{d15a5m_t34s3r_1e6a7731}

```

So, we don't need to look into another file(static.ltdis.x86_64.txt) for the flag more.

## Result
```Result :  1020 picoCTF{d15a5m_t34s3r_1e6a7731}```
